...
def load_sensitivity(sweep_root: str | Path) -> pd.DataFrame:
    ...
    for d in sorted(glob.glob(str(sweep_root / "results_sensitivity" / "S*"))):
        ...
        for f in glob.glob(f"{d}/UNIFIED_*.csv"):
            ...
            # Concat a metadata block in one operation: per-column inserts
            # (and .assign, which inserts internally) trigger pandas'
            # fragmentation PerformanceWarning on wide frames.
            meta = pd.DataFrame({"sweep": sweep, "platform": plat,
                                 "backend": backend, "dtype": dtype,
                                 "outlen": outlen, "model": model},
                                index=d2.index)
            rows.append(pd.concat([d2, meta], axis=1))
