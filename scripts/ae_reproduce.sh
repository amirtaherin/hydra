...
if (cd data/unified && sha256sum -c SHA256SUMS >/dev/null 2>&1); then
...
COLS=$(head -1 "$(find "$RESULTS_ROOT" -name 'UNIFIED_*.csv' | head -1)" | tr ',' '\n' | wc -l)
...
echo "                                telemetry aggregates ($COLS columns)"
